...
/*   Updated: 2023/03/11 00:34:49 by bpoumeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "mini_parsing.h"

static void	_init_first_child_fds(t_instruction_block_tree *first_block, int **pipe_arr);
...
t_ert	allocate_first_child_on(t_string_token **str_tok_pt, t_instruction_block_tree **block_pt, int **pipe_arr)
{
	...
	*block_pt = instruction_block_tree_constructor();
	if (*block_pt == NULL)
		return (MLC_ERR);
	_init_first_child_fds(*block_pt, pipe_arr);
